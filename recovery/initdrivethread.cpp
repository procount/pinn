...
bool InitDriveThread::method_resizePartitions()
{
    ...
    if (!QFile::exists(partdev(_drive, 1)))
    {
        ...
        emit query(tr("Would you like one to be created for you?\nWARNING: This will erase all data on your SD card"),
        ...
        if(answer == QMessageBox::Yes)
        {
            ...
            // suitably for installing and that they will have to
            // re-copy the files before rebooting
            emit error(tr("SD card has now been formatted ready for installation. Please re-copy the installer files onto the card and reboot"));
            return false;
        }
        else
        {
...
bool InitDriveThread::partitionDrive()
{
     ...
     * First logical partition has persistent settings partition
     */
    QByteArray partitionTable;
